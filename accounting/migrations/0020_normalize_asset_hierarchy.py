...
def normalize_asset_hierarchy(apps, schema_editor):
    ...
    for society in Society.objects.all():
        ...
        for leaf_name, parent_name in leaf_parent_map.items():
            leaf = first_by_name(leaf_name)
            parent = group_accounts.get(parent_name)
            if leaf and parent and leaf.parent_id != parent.id:
                leaf.parent = parent
...
class Migration(migrations.Migration):
    dependencies = [
        ("accounting", "0019_account_hierarchy_groups"),
    ]

    operations = [
        migrations.RunPython(normalize_asset_hierarchy, migrations.RunPython.noop),
    ]
